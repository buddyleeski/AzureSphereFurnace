...
*	20-40 탎 (1kOhm pullup pulls data to high)
*	80 탎 low + 80 탎 high initiates data transfer
*	50 탎ec low pulse from sensor initiates bit
*	26-28 탎ec ==> "0"
*	70 탎ec ==> "1"
...
DHT_SensorData* DHT_ReadData(GPIO_Id gpioPin)
{
	...
	// the pullup resistor draws the line to high. The protocol specs 20-40탎 in high state 
	// until the DHT pulls it low for 80탎 and high for 80탎 each to initiate data transfer
	// since closing/re-opening the GPIO port as input takes ~30탎 we may have already missed the initial high->low transition
	...
	for (uTransitions = 0; uTransitions < MAX_TRANSITIONS; uTransitions++) {
		/* Azure Sphere Timing: 1 loop iteration (count) is ~3탎 */
		...
		/* ignore first 2 transitions: 80탎 low -> 80탎 high -> 50탎 low */
